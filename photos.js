...
function rightArrow() {
   clearInterval(autoAdvance); //clear the timer 
   rightAdvance();
}
   ...
   if (showAllButton.addEventListener) {
    ...
    window.addEventListener('message',
        function () {
            ...
            favDiv.appendChild(divImage);

            var imgage = document.createElement('img');
            imgage.src = figFilenameSm;
            imgage.width = 170;
            divImage.appendChild(imgage);

            var paragraph = document.createElement('p');
            divImage.appendChild(paragraph);
        
            var remove = document.createElement('button');
            remove.type = 'button';
            remove.style.width = "170px";
            remove.style.border = 0;
            remove.style.padding = "5px";
            remove.innerText = 'Remove';
            remove.addEventListener('click',
                    function () {
                        favDiv.removeChild(divImage);
                    }
            );
            paragraph.appendChild(remove);

            if (favDiv.children.length > 5) {
                var zoomWindow = window.open('', "zoomwin");
                zoomWindow.focus();
                zoomWindow.document.getElementById("addFavourite").innerHTML = "Maximum number of favourites is 5";
            }
        }
    );
    ////////////////////////////////////////////////////////////
